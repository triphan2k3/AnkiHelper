...
def parse_header(subtree, PARAMS) -> list:
    # GET WORD
    header = None
    uk_pron = ""
    us_pron = ""
...
def parse_meaning(subtree, PARAMS) -> list:
...
def extract_infor(file_path, agrs):
    ...
    word_info['uk_pron_filename'] = "" if not word_info['uk_pron'] else word_info['id'] + "_uk.mp3"
    word_info['us_pron_filename'] = "" if not word_info['us_pron'] else word_info['id'] + "_us.mp3"
    ...
    meaning_infor = parse_meaning(subtree, PARAMS)
...
def parse_html(args):
    try:
        ...
        print("json directory already exists, makedirs skipped")

    files = os.listdir(args.html_dir)
    print_mp3_text = ""
    ...
    for file in files:
        ...
        if args.print_mp3:
            if word_info['uk_pron']:
                print_mp3_text += word_info['uk_pron'] + " " + word_info['uk_pron_filename'] + "\n"
                # print(word_info['uk_pron'], word_info['uk_pron_filename'])
            if word_info['us_pron']:
                print_mp3_text += word_info['us_pron'] + " " + word_info['us_pron_filename'] + "\n"
                # print(word_info['us_pron'], word_info['us_pron_filename'])

        print("Done!")
    if args.print_mp3:
        with open(args.filename_file, "w") as f:
            f.write(print_mp3_text)